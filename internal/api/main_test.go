...
// same database (currently this one and internal/ui) do not race for
...
func runWithDBLock(m *testing.M) int {
	...
	if dsn == "" {
		// No DB available; DB-backed tests will skip themselves.
		return m.Run()
	}
	...
	// db.Close() (deferred) releases the advisory lock by closing
